...
/*   Updated: 2022/11/29 19:02:57 by dasereno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_cast(int i, t_obj *player, t_obj *my_player)
{
	...
	_pc()->sprite_screen_x = (int)((WIN_W / 2) * (1 + _pc()->trans.x
			/ _pc()->trans.y));
	_pc()->move_screen = (int)((int)player->shott[i].pos.z / _pc()->trans.y)
	+ my_player->pitch + my_player->z / _pc()->trans.y;
}
			...
			&& _pc()->trans.y < _var()->zbuffer[stripe])
		{
			...
			while (y < _pc()->draw_end.y)
			{
				...
				_pc()->tex.y = ((_pc()->d * _image()->bullet.h)
					/ _pc()->size.y) / 256;
				ft_put_pixel(_img(), &_image()->bullet, (t_vector2D)
				{stripe, y}, _pc()->tex);
				y++;
			}
		}
