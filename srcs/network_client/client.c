...
//			ft_play_sound(i);
		...
		if (i == _player()->id)
		{
			...
			_player()->health = serv.player[i].health;
			_player()->shooted.shoot = serv.player[i].shooted.shoot;
			_player()->is_dead = serv.player[i].is_dead;
			_player()->shooted.id = serv.player[i].shooted.id;
		}
