...
/*   Updated: 2022/12/05 18:41:38 by dasereno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((pos_b.x * 4 + pos_b.y * b->line_length + 3 > b->w * 4
			+ b->h * b->line_length) || (pos_a.x * 4 + pos_a.y * a->line_length
			> a->w * 4 + a->h * a->line_length)
		|| (pos_a.x < 0 || pos_a.x < 0 || pos_b.x < 0 || pos_b.y < 0))
		return ;
	if (b->addr[((pos_b.y * b->line_length) + (pos_b.x * 4)) + 3] != -1)
	{
		a->addr[pos_a.y * a->line_length + pos_a.x * 4]
			= b->addr[(pos_b.y * b->line_length) + (pos_b.x * 4)];
